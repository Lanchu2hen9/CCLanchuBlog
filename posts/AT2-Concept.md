...
So yesterday on the 1 May 2025, I think I was just confused with what I wanted for AT2 in terms of sound. Now I think my code is doing what I want it to do in terms of the audio. The audio currently stops playing, when the user stops moving their mouse for a while. This is because the `MouseTracker();` triggers the audio clips to blend/it triggers the `AudioBlender();`[^4] function. And everytime the mouse moves[^3] it triggers a new instance.

So for example, when first time moving it goes:

1. `MouseTracker();` tracks the position, and proportion of the mouse in relation to the (0,0) point.
2. `AudioBlender();` is called within `MouseTracker();`, so it runs after the position of the mouse is tracked.
   And then it repeats from Step 1 and 2 each time the code runs.

Strangly enough, I like this unintented function of the code. Because I think this adds to my own idea of "Processing ≠ Thinking", "hearing ≠ listening". See [this section](#how-does-this-relate-to-at2). I believe a viewer/actor's interaction within a system at large, adds both understanding and chaos to them and the system.

### My thought process:

1. The universe is large, and humans seek to understand it due to their "sapience".
2. Humans learn more about how stars, gravity, astronomy.
3. The wide expense of the universe decentres that "human-centred ideal of the past".
4. Humans are big confused and try to find "pattern"/more life in the universe like them. => Adds "chaos" to the internal system[^5] of humanity.
5. That drive to seek out more information and to be more curious.

In summary, the universe is an already pre-existing system that seems "ordered" at first glance, but can be "chaotic" if you examine it further. And due to humanity's curiousity and drive to learn more, they wish to "interact" with the "system"-at large.

How this relates to my net art, peaceful stars at first, system is suspended seems ordered, user input => creates chaos. Like how the universe seems ordered at first, and human "interaction"/"understand" adds chaos to the system.

**Sidenote** I may need to delay the sounds played more.
...
function MouseTracker() {
  cnv.addEventListener("mousemove", (e) => {
    ...
    AudioBlender(blendXProportion, blendYProportion);
  });
}
//#endregion

//#startregion AudioBlender
function AudioBlender(blendXProportion, blendYProportion) {
  let SoundX =
    XDistance >= 0
      ? XSoundsStart[Math.floor(Random(0, XSoundsStart.length))]
      : XSoundsEnd[Math.floor(Random(0, XSoundsEnd.length))];
...
Because, I'm like already delaying the "crazy" audio to be played on the user's click. I will be re-using the above code, if I do want to create some sort of "start" page.
...
class LigthningStrikes {
  ...
  createChild() {
    if (this.length > 55 && this.generation < 4) {
      // Maybe make the lightning longer?
...
[^3]: `MouseTracker();` is the function responsible for keeping track of where the mouse's position is in.
[^4]: `AudioBlender();` Chooses a random audio, plays it then blends the volume. Probably not the best name to call this function but eh.
[^5]: When I say system I mean both systems of understanding, where the participants within the systems create understanding/information with the other members/actors of the system. For examples, the Creative Coding class is a system, you have the actors/members within that system that see to create understanding and more information with the others actors/members of the system.
